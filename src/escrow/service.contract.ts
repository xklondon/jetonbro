...
import { type EscrowService } from './service.js';
...
async function fundedTable(createService: () => EscrowService | Promise<EscrowService>) {
  const escrow = await createService();
...
async function confirmAndLock(service: EscrowService, amount = 20) {
...
export function defineEscrowServiceTests(
  label: string,
  createService: () => EscrowService | Promise<EscrowService>,
): void {
describe(`EscrowService (${label})`, () => {
  ...
  it('fails when locking more than the game wallet balance', async () => {
    const service = await fundedTable(createService);
    const confirmed = await service.confirm({
      userId: PLAYER,
      tableId: TABLE,
      amount: 51,
